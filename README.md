# 🍉 **통통통**!

<img width="4088" height="2080" alt="리드미_썸네일" src="https://github.com/user-attachments/assets/f012c813-bd9c-4731-9452-ca9050b3e5e9" />

</br>

## 📱 소개

> **수박 고르기가 어려운 사람들을 위해, '통통통' 두드리는 소리로 AI가 숙성도를 예측하는 iOS 앱 - 통통통🍉**
> 

</br>

## 📆 프로젝트 기간

- 전체 기간: `2025.06.23 - 2025.08.01`
- 개발 기간: `2025.07.09 - ing`

</br>

## 🛠 기술 스택

- **iOS**
    
    - Swift / SwiftUI / UIKit
    
    - MVVM Architecture
    
    - AVFoundation
    
- **Backend**
    
    - Python / FastAPI / Ubuntu
      
      - https://github.com/ohcuy/tongtongtong-api
      - https://github.com/cheshire0105/wm_mlt_models-main
- **ML**
    
    - Python / CreateML / CoreML
    
      - https://github.com/sing-u8/watermelon_vgg16_model
      - https://github.com/sing-u8/Watermelon_ML_Models
      - https://github.com/sing-u8/wm_mlt_models
 
</br>

## 🌟 주요 기능

1. 수박을 손 끝으로 3번 두드려 '통통통' 소리를 녹음한다.
2. 녹음된 소리를 통해 잘 익은 수박 / 덜 익은 수박 / 많이 익은 수박을 감별한다.

</br>

## 🧑‍💻 팀 소개

| 이름 | 역할 | GitHub |
| --- | --- | --- |
| Bear | iOS Developer | [bear3283](https://github.com/bear3283) |
| Brandnew | iOS Developer / Backend Developer | [sing-u8](https://github.com/sing-u8) |
| Cheshire | iOS Developer / Backend Developer Designer | [cheshire0105](https://github.com/cheshire0105) |
| Leo | iOS Developer | [Juncheoltree](https://github.com/Juncheoltree) |
| Mosae | Designer | [uvo36](https://github.com/uvo36) |
| Yuu | iOS Developer / Backend Developer | [ohcuy](https://github.com/ohcuy) |

</br>

## 📎 프로젝트 문서

[수박 당도 예측 모델 Wiki](https://github.com/sing-u8/Watermelon_ML_Models/wiki)
